...
const MAP_TILE_FADE_DURATION_MS = (() => {
  const parsed = Number.parseInt(String(process.env.NEXT_PUBLIC_MAP_TILE_FADE_DURATION_MS || ''), 10)
  if (!Number.isFinite(parsed)) return 180
  return Math.max(0, Math.min(600, parsed))
})()
const MAP_TILE_CACHE_ZOOM_LEVELS = (() => {
  const parsed = Number.parseInt(String(process.env.NEXT_PUBLIC_MAP_TILE_CACHE_ZOOM_LEVELS || ''), 10)
  if (!Number.isFinite(parsed)) return 8
  return Math.max(2, Math.min(12, parsed))
})()
const MAP_KEEP_PENDING_TILE_REQUESTS_DURING_ZOOM = String(
  process.env.NEXT_PUBLIC_MAP_KEEP_PENDING_TILE_REQUESTS_DURING_ZOOM || '1'
).trim() !== '0'
...
const COMPLETE_WINDOW_EXCERPT_MIN_ZOOM = 12
const COMPLETE_WINDOW_POINT_EXCERPT_MAX = 9
const COMPLETE_WINDOW_BANGUMI_EXCERPT_MAX = 7
...
const L: Record<SupportedLocale, Record<string, string>> = {
  zh: {
    ...
    stateAutoHint: '加入「我的地图」后可规划巡礼路线、保存旅行计划、站内导航。',
  ...
  en: {
    ...
    stateAutoHint: 'Add to My Map to plan pilgrimage routes, save travel plans, and navigate in-app.',
  ...
  ja: {
    ...
    stateAutoHint: 'マイマップに追加してルートを計画、旅行プランを保存、アプリ内でナビ。',
...
export {
  ...
  COMPLETE_AVATAR_MAX_ZOOM, COMPLETE_DETAIL_THEME_MIN_ZOOM, COMPLETE_DETAIL_THEME_MAX_ZOOM,
  COMPLETE_WINDOW_EXCERPT_MIN_ZOOM, COMPLETE_WINDOW_POINT_EXCERPT_MAX, COMPLETE_WINDOW_BANGUMI_EXCERPT_MAX,
  resolveImageBuildZoom, resolveImageShowZoom,
