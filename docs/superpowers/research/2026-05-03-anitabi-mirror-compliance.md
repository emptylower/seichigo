...
- Trigger: `both` for the sync workflow. `runAnitabiSync()` is invoked by the Vercel cron route at `app/api/cron/anitabi/daily/route.ts` via `lib/anitabi/handlers/cron.ts`, by the manual CLI entry `npm run anitabi:sync` via `scripts/anitabi-sync.ts`, and by the admin sync handler in `lib/anitabi/handlers/adminSync.ts`. Separately, `lib/anitabi/sync/diff.ts` is not the sync runtime entrypoint; `buildAnitabiSyncDiffSummary()` is used by the admin diff inspection endpoint in `app/api/admin/anitabi/diff/route.ts` / `lib/anitabi/handlers/adminDiff.ts`.
