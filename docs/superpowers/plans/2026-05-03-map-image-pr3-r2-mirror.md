...
Reserved source types for `MapImageMirrorState`:
```ts
const META_SOURCE_TYPES = ['__throttle__', '__cursor__'] as const
```
Rows whose `sourceType` is in `META_SOURCE_TYPES` are meta-state markers for the breaker / delta watermark, not real mirror records. Keep direct `THROTTLE_KEY` / `CURSOR_KEY` lookups intact, but every aggregate/status query added later in Task 4.2, Task 4.3, and Task 5.1 must filter them out with Prisma `where: { sourceType: { notIn: META_SOURCE_TYPES } }` (or the SQL equivalent).
...
export async function GET(req: Request) {
  ...
  const recentFailures = await deps.prisma.mapImageMirrorState.findMany({
    where: { status: 'failed', sourceType: { notIn: ['__throttle__', '__cursor__'] } },
...
The Task 4.2 status endpoint is the aggregation boundary for the dashboard and CLI snapshot. Every `groupBy`, `count`, or SQL aggregate behind this endpoint must exclude the reserved meta rows so progress totals only reflect real bangumi / point mirror records.
...
Task 4.3 should consume the already-filtered Task 4.2 status aggregates as-is. Do not add client-side adjustments for `__throttle__` / `__cursor__`; the endpoint query layer is responsible for excluding those meta rows from progress totals and rate calculations.
...
Task 5.1 mostly does row-specific resets / upserts, so it should not apply the meta-row filter to these direct bangumi / point writes or to the existing `THROTTLE_KEY` / `CURSOR_KEY` lookups elsewhere in the plan. The reserved-source filter only applies if this reconcile path later adds aggregate progress/status queries over `MapImageMirrorState`; in that case, use `where: { sourceType: { notIn: ['__throttle__', '__cursor__'] } }` or the SQL equivalent.
