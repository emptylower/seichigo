...
| 标准 | 开放购买 | $9.9（2026-09-06 拍板） | $9.9 × 45% = $4.455 | 完整的当前功能 |
| 高级 | 占位，不可订阅 | 不显示价格，展示“即将推出” | 内测预算按 $29.9 × 45% | 全部功能解锁、天数不限、贵 API 与多模型 |

45% 的成本占比留出 5 个点给支付手续费、退款与 Worker、Neon 摊销，对应用满时毛利 55%。定价以美元计价。标准档 $9.9 的宣传口径是“每月约 15 次行程规划”：按中性成本假设（单次行程约 $0.40）预算够 11 次重 run 加追问，起步阶段处在 Google 月免费额度内实际远超 15 次；计量层跑满两周后按 §10 复核，若落在保守区间则把口径收紧到 10 次或调价。
...
- 定价页列出三档，高级档卡片可见、按钮置灰、标注“即将推出”，功能列全部显示为解锁。站点页眉、页脚与首页有“套餐”入口指向定价页。
- 升级提示覆盖所有“升级可解锁”的位置：交通行的估算徽标、餐厅占位卡、地图上直线示意徽标旁、行程天数达到档位上限时的日程标题区、用量耗尽提示条。每处都是一个指向定价页的小链接，文案只说解锁什么，不解释机制。
