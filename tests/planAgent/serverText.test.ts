...
describe('serverText 三语字典', () => {
  ...
  it('status / summary / result / errors / meal 段都在', () => {
    for (const locale of ['zh', 'en', 'ja'] as SupportedLocale[]) {
      ...
      expect(d.status.readHistory).toBeTruthy()
      expect(d.status.checkProgress).toBeTruthy()
      expect(d.status.organize).toBeTruthy()
  ...
  it('zh 文案与现有原文逐字一致（抽查关键句）', () => {
    ...
    expect(d.status.readHistory).toBe('正在读取对话历史')
    expect(d.status.checkProgress).toBe('正在核对行程进度')
    expect(d.status.organize).toBe('正在组织思路')
