    ...
    <div className="space-y-16 pb-12">
        ...
        <div className="grid gap-12 lg:grid-cols-2 lg:items-center max-w-7xl mx-auto">
          <div className="space-y-8 text-center lg:text-left">
            ...
            <div className="flex flex-wrap justify-center lg:justify-start gap-4 pt-2">
              ...
              >
                {t('pages.home.browseAnimeButton', 'en')}
              </Link>
              <Link
                href="/en/submit"
          ...
          <div className="relative hidden lg:block h-[400px]">
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md h-80">
              {heroDisplay.map((item, i) => {
                ...
                const altText = item.name ? `${item.name} cover` : 'Anime cover'
                return (
                  <div
                    key={i}
      ...
      {featured ? (
        <section className="space-y-6 max-w-7xl mx-auto px-6">
           <div className="flex items-center gap-2 px-1 border-l-4 border-brand-500 pl-3">
             <h2 className="text-2xl font-bold tracking-tight text-gray-900">{t('pages.home.featuredSectionTitle', 'en')}</h2>
          </div>
           <FeaturedPost item={featured} locale="en" />
        </section>
      ) : null}
      ...
      <section className="space-y-6 max-w-7xl mx-auto px-6">
        <div className="flex items-center justify-between px-1 border-l-4 border-brand-500 pl-3">
          <h2 className="text-2xl font-bold tracking-tight text-gray-900">{t('pages.home.latestSectionTitle', 'en')}</h2>
          <Link href="/en/anime" className="text-sm font-medium text-brand-600 hover:text-brand-700">
            {t('pages.home.viewAllAnimeLink', 'en')}
          </Link>
        </div>
        <div className="-mx-4 px-4 sm:mx-0 sm:px-0">
          <BookShelf items={latestShelf} locale="en" />
        </div>
      </section>
      ...
      {more.length ? (
        <section className="space-y-6 max-w-7xl mx-auto px-6">
          <div className="flex items-center justify-between px-1 border-l-4 border-brand-500 pl-3">
            <h2 className="text-2xl font-bold tracking-tight text-gray-900">{t('pages.home.exploreSectionTitle', 'en')}</h2>
            <Link href="/en/anime" className="text-sm font-medium text-brand-600 hover:text-brand-700">
              {t('pages.home.goToAnimeIndexLink', 'en')}
            </Link>
          </div>
      ...
      <section className="max-w-7xl mx-auto px-6">
        <div className="relative overflow-hidden rounded-3xl bg-gray-900 px-6 py-12 shadow-2xl sm:px-12 md:py-16">
          {/* Background decorative glow - using brand colors for atmosphere */}
          ...
          <div className="relative z-10 flex flex-col items-center justify-between gap-8 md:flex-row">
            <div className="flex-1 space-y-6 text-center md:text-left">
              <div className="flex flex-col items-center gap-5 md:flex-row md:items-start">
                {/* Logo container - with glow and frosted effect */}
                ...
                <div className="space-y-2 pt-1">
                  <div className="flex items-center justify-center gap-3 md:justify-start">
                    <h2 className="text-3xl font-bold text-white tracking-tight">{t('pages.home.appPromoTitle', 'en')}</h2>
                    <span className="rounded-full bg-brand-500/20 px-3 py-1 text-xs font-bold text-brand-300 ring-1 ring-brand-500/50 backdrop-blur-md">
                      {t('pages.home.appPromoComingSoon', 'en')}
                    </span>
                  </div>
                  <p className="max-w-lg text-base leading-relaxed text-gray-300">
                    {t('pages.home.appPromoDescription', 'en')}<span className="text-white font-medium">{t('pages.home.appPromoOfflineMap', 'en')}</span>{t('pages.home.appPromoAnd', 'en')}<span className="text-white font-medium">{t('pages.home.appPromoNavigation', 'en')}</span>.
                    <br className="hidden sm:block" />
                    {t('pages.home.appPromoDescriptionLine2', 'en')}
                  </p>
                </div>
              </div>
            </div>
