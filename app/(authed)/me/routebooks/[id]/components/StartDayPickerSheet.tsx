...
export function StartDayPickerSheet({ open, days, items, onPick, onClose }: Props) {
  if (!open) return null
    ...
    <div className="fixed inset-0 z-[100] flex items-end justify-center">
      ...
      <div className="relative mb-0 w-full max-w-md rounded-t-[28px] border border-pink-100 bg-white p-4 pb-8 shadow-[0_-18px_44px_-24px_rgba(15,23,42,0.45)]">
        <div className="mb-3 flex items-center justify-between px-1">
          <h3 className="text-base font-semibold text-slate-900">从哪一天开始</h3>
          <button
            type="button"
            aria-label="关闭"
            ...
            return (
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  <span className="block text-sm font-semibold text-slate-900">{dayLabel(day, day.dayIndex)}</span>
                  {day.title ? <span className="block truncate text-xs text-slate-400">{day.title}</span> : null}
                </span>
                <span className="shrink-0 text-xs text-slate-400">
                  {stopCount > 0 ? `${stopCount} 站` : '还没有站点'}
                </span>
              </button>
            )
          })}
