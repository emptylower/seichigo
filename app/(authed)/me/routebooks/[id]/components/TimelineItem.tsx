...
export function TimelineItem({
  ...
  const title = itemDisplayTitle(item, preview, places, locale)
    ...
    if (item.kind === 'transit') {
      ...
      return (
        <div className="flex min-w-0 flex-1 items-center gap-2 text-xs text-slate-500">
          ...
          {durationMin !== null ? <span className="shrink-0">{tr('routebook.timeline.aboutMinutes', locale, { n: Math.round(durationMin) })}</span> : null}
          {staleTransit ? <span className="shrink-0 rounded-full bg-slate-200/70 px-1.5 py-0.5 text-[10px] text-slate-400">{tr('routebook.timeline.stale', locale)}</span> : null}
        </div>
      )
    }
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1.5">
              ...
              <span className="shrink-0 rounded-full bg-slate-100 px-1.5 py-0.5 text-[10px] text-slate-500">
                {tr(`routebook.placeKind.${place?.kind ?? 'other'}`, locale)}
              </span>
            </div>
  ...
  return (
    ...
    >
      ...
      {typeof seq === 'number' ? (
        <span
          aria-hidden
          className="mt-1.5 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-brand-500 text-[10px] font-bold text-white"
        >
          {seq}
        </span>
      ) : null}
      ...
      <div className="min-w-0 flex-1">
        {onOpenDetail && (item.kind === 'point' || item.kind === 'place') ? (
          <button
            type="button"
            aria-label={tr('routebook.timeline.viewItem', locale, { title })}
