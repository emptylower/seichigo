...
import { getOpsApiDeps } from '@/lib/ops/api'
import { createHandlers } from '@/lib/ops/handlers/reports'
...
export async function GET(req: Request) {
  try {
    const deps = await getOpsApiDeps()
    return createHandlers(deps).GET(req)
  } catch (err) {
    console.error('[api/admin/ops/reports] GET failed', err)
...
export async function POST(req: Request) {
  try {
    ...
    console.error('[api/admin/ops/reports] POST failed', err)
    return routeError(err)
  }
}
