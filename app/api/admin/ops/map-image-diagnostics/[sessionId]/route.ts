...
import { createHandlers } from '@/lib/mapImageDiag/handlers/adminSessions'
...
export async function GET(req: Request, ctx: { params: Promise<{ sessionId: string }> }) {
  try {
    ...
    return createHandlers(deps).GET_BY_ID(req, ctx)
  } catch (err) {
    console.error('[api/admin/ops/map-image-diagnostics/[sessionId]] GET failed', err)
...
export async function DELETE(req: Request, ctx: { params: Promise<{ sessionId: string }> }) {
  try {
    const deps = await getMapImageDiagApiDeps()
    return createHandlers(deps).DELETE_BY_ID(req, ctx)
  } catch (err) {
    console.error('[api/admin/ops/map-image-diagnostics/[sessionId]] DELETE failed', err)
    return routeError(err)
  }
}
