...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (override?.action === 'redirect') {
    return { title: 'コンテンツを移動しました', robots: { index: false, follow: false } }
  }
  if (override?.action === 'replace-with-emergency-copy') {
    return {
      title: override.title || '緊急告知',
      description: override.bodyText || 'コンテンツを一時差し替え中です',
  ...
  return {
    title: hasJaTranslation ? seoTitle : { absolute: `${seoTitle.absolute} (中国語)` },
    description,
    ...
    openGraph: {
      ...
      images: [pageCardImage('post', frontmatter.slug, 'ja', String(frontmatter.title || 'SeichiGo'))],
    },
    twitter: {
      ...
      <div key={canonicalSlug} className="mx-auto w-full max-w-7xl px-6 lg:px-10" data-layout-wide="true">
        <div className="flex items-start gap-12">
          ...
          <main className="min-w-0 flex-1 pb-24">
            ...
            {tags?.includes('seo-spoke') && <AiAssistedNotice locale="ja" />}
