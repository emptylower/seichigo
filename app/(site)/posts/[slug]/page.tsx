...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (override?.action === 'redirect') {
    return { title: '内容已迁移', robots: { index: false, follow: false } }
  }
  if (override?.action === 'replace-with-emergency-copy') {
    return {
      title: override.title || '紧急公告',
      description: override.bodyText || '内容临时调整中',
  ...
  return {
    ...
    openGraph: {
      ...
      images: [pageCardImage('post', frontmatter.slug, 'zh', String(frontmatter.title || 'SeichiGo'))],
    },
    twitter: {
  ...
  const anime = await Promise.all(
    animeIds.map(async (id) => {
      const meta = await getAnimeById(id).catch((error) => {
        console.error('[degraded:post-detail.anime-label]', { locale: 'zh', slug, id }, error)
        return null
      })
