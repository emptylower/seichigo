...
export default function ArticleComposerClient({ initial }: Props) {
  ...
  async function flushSave(reason: FlushReason) {
    ...
    try {
      ...
      if (succeeded && pendingSave.current && pendingSave.current !== lastSaved.current) {
        ...
        // On failures: rely on retryTimer (for transient errors) or user edits/manual retry.
        if (!retryableFailure) clearTimer(debounceTimer)
      }
    }
  }
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error('autosave: payload serialize failed', err)
      setSaveError('保存失败（内容序列化异常）')
      setSaveState('error')
      return
    }
    if (!next) return
    ...
    return () => {
      // No-op: the latest debounced timer will be cleared/replaced on next change.
    }
  }, [editable, id, payload])
