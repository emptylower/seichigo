...
export function buildGpx(input: GpxExportInput): string {
  ...
  for (const item of scopedItems) {
    if (item.kind === 'point' && item.pointId) {
      ...
      wpts.push(wpt({ lat: preview.lat, lng: preview.lng, name: preview.title, desc: item.note ?? undefined, sym: 'point' }))
    }
  ...
  for (const day of scopedDays) {
    ...
    for (const item of dayItems) {
      if (item.kind === 'point' && item.pointId) {
        const preview = previews.get(item.pointId)
        if (!preview) continue
        stops.push({ lat: preview.lat, lng: preview.lng, name: preview.title })
      } else if (item.kind === 'place' && item.placeId) {
        const place = placeById.get(item.placeId)
        if (!place) continue
        stops.push({ lat: place.lat, lng: place.lng, name: place.title })
        scopedPlaceIdsForRte.add(place.id)
      }
    }
    if (anchor.end && !(anchor.start && anchor.start.placeId === anchor.end.placeId)) {
