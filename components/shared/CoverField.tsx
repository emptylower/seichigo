...
async function encodeCoverFromCrop(opts: {
  ...
  const baseOutH = Math.max(1, Math.round(baseOutW / opts.targetAspect))
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    outH = Math.max(1, Math.round(outW / opts.targetAspect))
  }
...
export default function CoverField({ 
  ...
  async function onPickFile(nextFile: File) {
    ...
    const needsCrop = !Number.isFinite(ratio) || Math.abs(ratio - aspectRatio) > RATIO_TOLERANCE
  ...
  async function onUrlSubmit() {
    ...
    setUploading(true)
    try {
      if (onSave) await onSave(raw)
      onChange(raw)
      setInputUrl('')
      setShowInput(false)
    } catch (err: any) {
      setError(String(err?.message || '保存失败'))
    } finally {
      setUploading(false)
    }
  }
    ...
    <div className="space-y-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {showInput ? (
            <div className="flex items-center gap-2">
              ...
              />
              <Button type="button" onClick={onUrlSubmit} disabled={Boolean(disabled || uploading)}>
                确定
              </Button>
              <Button type="button" variant="ghost" onClick={() => setShowInput(false)} disabled={Boolean(disabled || uploading)}>
                取消
              </Button>
            </div>
          ) : (
            <>
              <Button type="button" variant="ghost" onClick={openFileDialog} disabled={Boolean(disabled || uploading)}>
                {uploading ? '处理中…' : value ? '上传更换' : '上传图片'}
              </Button>
              <Button type="button" variant="ghost" onClick={() => setShowInput(true)} disabled={Boolean(disabled || uploading)}>
                {value ? '输入 URL' : '输入 URL'}
              </Button>
            </>
          )}
      ...
      <div className="flex gap-4">
        <div className="w-40 shrink-0">
          <div className="relative overflow-hidden rounded-lg border bg-gray-50" style={{ aspectRatio }}>
        ...
        <div className="min-w-0 flex-1 text-sm text-gray-600">
          ...
          <div className="mt-1">如果上传比例不符合，会弹窗让你裁剪；输入 URL 则直接使用。</div>
        </div>
      </div>
          ...
          <div className="flex max-h-[calc(100dvh-2rem)] w-full max-w-2xl flex-col overflow-hidden rounded-xl bg-white shadow-xl">
            <div className="flex items-start justify-between gap-4 border-b px-5 py-4">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-600">拖动图片调整位置，使用滑块缩放。</p>
              </div>
              <button
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
              <div className="grid gap-4 md:grid-cols-[1fr,260px] md:items-start">
                <div className="rounded-lg border bg-gray-50 p-3">
                  <div
                    ref={cropFrameRef}
                    className="relative mx-auto w-full max-w-[560px] overflow-hidden rounded-md bg-black"
                    style={{ aspectRatio }}
